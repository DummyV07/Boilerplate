...
## 🧩 从 PoC / SOP 到工业级：骨架不变，血肉升级

> *7 月 20 日感悟 · 全栈 AI 工程视角*

模版本身就是一套可跑通的 **SOP 骨架**。真正上线时，忌讳为了高并发推倒重来；正确姿势是 **架构高度复用，中间件与通信机制渐进加装**。

### 两个常挂嘴边的词

| 术语 | 全称 | 含义 | 在本仓库里的位置 |
| :--- | :--- | :--- | :--- |
| **PoC** | Proof of Concept（概念验证） | 用最低成本证明「这事儿能办成」，不追求完美与高并发 | Fork 后第一周跑通的 Demo / 演示 |
| **SOP** | Standard Operating Procedure（标准作业程序） | 把可复用的研发链路规范化、标准化，后续项目对照加固 | 本模版的分层、契约、可观测性接口 |

**关系一句话：** 先用极快的 **PoC** 验证业务可行性，再把链路沉淀成 **SOP 代码框架**；上线项目对照这套标准流程做工业级加固即可。

> **跑通** 与 **没跑通** 之间隔着大部分难度；**跑通** 到 **生产可用** 主要是工程加固，不是换底盘。

### 全栈视角：用工程组合拳降级算法难题

纯套框架的应用开发容易做出「能连通、但一脏数据就崩」的系统；纯算法又容易写出「指标好看、上不了线」的脚本。全栈价值在交界点：

- **输入端卡死格式**：能用 Vue 表单约束、正则、结构化 Schema 解决的，不要全扔给大模型。
- **重活与查询解耦**：文档解析 / 抽取等长耗时写入，绝不堵在用户查询的 HTTP 路径上。
- **模块可替换**：业务拓扑（Schema、API Contract、图结构）尽量不动；换的是执行方式（同步 → 队列、整包返回 → SSE 流式）。

### SOP → 落地：加装外挂，而不是换车

| SOP 里的基础实现（模版现状） | 通关时的「外挂」 | 为什么能直接改 |
| :--- | :--- | :--- |
| 同步 / 请求内长耗时逻辑 | → 丢进 Celery / 消息队列异步消费 | 入参与算法不动，只换执行场所 |
| 直连图库 / 向量库查询 | → 外套超时、熔断、降级到纯向量检索 | Cypher / search 原逻辑可保留 |
| 普通 HTTP 整包返回 | → `StreamingResponse`（SSE）流式吐字 | 生成逻辑不变，`return` 改 `yield` |
| 单通道大模型 API | → LiteLLM / 网关多通道 + Rate Limit 切换 | 业务层只认统一调用接口 |

**模块化的意义：** 某天要把同步写入改成消息队列，理想情况下只重写那一个业务 Module，前端契约与核心 Schema 不动。

### 工业级四道防线（按业务量分步加）

面向 Graph RAG / AI 流水线这类 **重计算 + 重 I/O + 依赖外部推理** 的系统，从「能演示」到「能抗打」至少盯这四层：

#### 1. 高并发：别让推理与写入堵死请求路径

- 全链路 `async/await`；对用户侧优先 **SSE 流式响应**（先出字，再补全）。
- **写入（Ingestion）走队列**：上传后立刻返回 `processing`，Worker 慢慢啃再写库；查询路径保持轻。
- 图库 / 向量库客户端配置合理 **连接池**，避免高并发下端口耗尽。

#### 2. 图检索：防「超级节点」拓扑风暴

- 每条图查询必须有 **LIMIT** 与 **TIMEOUT**（宁可召回不全，也不让单次查询拖垮集群）。
- 按节点度数 **动态降深**：超级节点自动 1-hop 或改属性过滤，避免 2-hop 爆炸。

#### 3. 容灾：默认一切都会挂

- **混合召回降级**：图库超时 → 自动退回纯向量检索，返回「能用」的答案，而不是 500。
- **模型通道降级**：API 限流 / 抖动 → 切备用通道或本地实例。
- **双写补偿**：图写入成功但向量失败时要有重试；可用夜间 Cron 做两侧数据对账补齐。

#### 4. 可观测性：别只靠翻 Terminal

- 模版已预留 `/metrics`、`/health/{node}`、`trace_id`——上线后对齐 Grafana。
- AI 链路建议再接 **Langfuse / Phoenix** 一类 Trace：一眼看到检索 200ms、图查 1.2s、生成 3.5s，定位瓶颈而不是猜。

### 实操优先级（别一次堆完）

| 时机 | 先做什么 | 挡住什么 |
| :--- | :--- | :--- |
| **上线前夕** | SSE 流式 + 图查询 LIMIT/TIMEOUT + 模型/检索 Try-Catch 降级 | 约 80% 的线上直接崩溃 |
| **数据量破百万 / 写入变重** | RabbitMQ/Kafka + Celery 解耦写入；Langfuse 等做性能调优 | 吞吐与排障能力 |

**口诀：PoC 证可行 → SOP 定标准 → 外挂式加固上生产。** 过度设计（Over-engineering）和推倒重来，一样危险。
